...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = gamma*xhat+beta
        cache=(gamma,mean,variance,xhat,x,eps)
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    (gamma,mean,variance,xhat,x,eps)=cache
    dgamma = np.sum(dout*xhat, axis=0)
    dbeta = np.sum(dout, axis=0)
    dxhat = dout*gamma
    assert (dxhat.shape == (N,D)), "Shape of dxhat invalid"
    dvariance = -0.5*np.sum(dxhat*xhat*np.power(variance+eps,-1),axis=0)
    assert (dvariance.shape == (D,)), "Shape of dvariance invalid"
    dmean = dvariance*np.mean(-2*(x-mean),axis=0) - np.sum(dxhat*np.power(variance+eps,-0.5),axis=0)
    dx = dxhat*np.power(variance+eps,-0.5) + dvariance*2*(x-mean)/N + dmean/N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, axis=0)
    dxhat = dout*gamma
    dx = (1/N)*np.power(variance+eps,-0.5)*\
...
def layernorm_backward(dout, cache):
    ...
    xhat = xhat.T
    N,D =  xhat.shape
    dx = (1/N)*np.power(variance+eps,-0.5)*(N*dxhat - np.sum(dxhat, axis=0)- xhat*np.sum(dxhat*xhat, axis=0))
    dx = dx.T
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
